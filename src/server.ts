...
app.get("/jokes/:id", (req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    if (!type) {
      return res.status(400).json({
        message: "Sorry, you have to enter the type query parameter.",
      });
    }
    const allJokesWithType = jokes.filter((joke) => joke.jokeType === type);
    if (allJokesWithType.length === 0) {
      return res
        .status(404)
        .json({ message: `Sorry, no jokes found with type: ${type}.` });
    }
    return res.status(200).json(allJokesWithType);
  } catch (error) {
    return res.status(500).json({
      message:
        error instanceof Error
...
app.post("/jokes", (req: Request, res: Response) => {
  try {
    const { text, type } = req.body; //解构req.body obj，只提取出text, type property
    if (!text || !type) {
      return res.status(400).json({
        message: "Both of text and type are required to post a joke.",
      });
    } else {
      const newJoke: Joke = {
        id: jokes.length + 1,
        jokeText: req.body.text.toString(),
        jokeType: req.body.type.toString(),
      };
      jokes.push(newJoke);
...
app.put("/jokes/:id", (req: Request, res: Response) => {
  try {
    const joke = jokes.find((joke) => joke.id === parseInt(req.params.id));
    if (!joke) {
      return res
        .status(404)
        .json({ message: `There is no current joke with id ${req.params.id}` });
    }
    const { text, type } = req.body;
    if (!text || !type) {
      return res
        .status(400)
        .json({ message: "Both text and type are required." });
    }
    joke.jokeText = text.toString();
...
app.patch("/jokes/:id", (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
    ...
    if (index === -1) {
      return res
        .status(404)
        .json({ message: `There is no such joke with the ID ${id}.` });
    }
    const { text, type } = req.body;
    const currentJoke = jokes[index];
    const replacementJoke: Joke = {
      id: id,
      jokeText: text ? text.toString() : currentJoke.jokeText,
      jokeType: type ? type.toString() : currentJoke.jokeType,
    };
